...
contract SushiSwapMultiSwapper {
    ...
    constructor (address _factory, ICoffinBoxV1 _coffinBox, bytes32 _pairCodeHash) public {
        factory = _factory;
    ...
    function swap (IERC20 tokenIn, IERC20 tokenOut, uint256 amountMinOut, address[] calldata path, uint256 shareIn) external returns (uint256 amountOut, uint256 shareOut) {
        (uint256 amountIn, ) = coffinBox.withdraw(tokenIn, address(this), address(this), 0, shareIn);
        amountOut = _swapExactTokensForTokens(amountIn, amountMinOut, path, address(coffinBox));
        (, shareOut) = coffinBox.deposit(tokenOut, address(coffinBox), msg.sender, amountOut, 0);
    }
